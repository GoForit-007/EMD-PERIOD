...
# load data
data_path = '../EMD-PERIOD/data/A4Benchmark'
file_name_list = []
file_list = os.listdir(data_path)
for i in file_list:
    if os.path.splitext(i)[1] == '.csv':
        if i != 'A4Benchmark_all.csv':
...
for i in tqdm(range(fileLength)):
    ...
    emd = EMD()
    IMF = emd(y_emd)
    YahooA4_EMD[file_name.split('.')[0]] = np.concatenate((y_emd.reshape(1, -1), IMF), axis=0)
...
for key, value in YahooA4_EMD.items():
    print('--', key, '--')
    y = value[0, :]
    IMF = value[1::, :]
    IMF_LBtest = []
    ...
    for i, imf in enumerate(IMF):
        if IMF_LBtest[i] == 0 or IMF_permutationEntropy[i] > 0.9:
            period_fft = -2
...
for key, value in IMF_period_dict.items():
    ...
    bool_permutationEntropy = IMF_permutationEntropy_dict[key] < 0.9
...
file.save('EMD_YahooA4_result.xls')
